  ...
  async bulkProductSave(event: Event) {
    ...
    if (this.tableArray.length !== 0) {
      ...
      for (let table of this.tableArray) {
        ...
        const sanitizedCategory = category.trim() !== '' ? category : null;
        const sanitizedSubCategory = subCategory.trim() !== '' ? subCategory : null;
        const sanitizedSubSubCategory = subSubCategory.trim() !== '' ? subSubCategory : null;
  ...
  async verifyDetails() {
    ...
    for (const table of this.tableArray) {
      ...
      setTimeout(async () => {
        ...
        let mainCatHasSub = false;
        let subCatHasSubs = false;
        //check if main category has sub cats
        if (this.findCaseInsensitiveFromObj(table, 'Category') !== undefined) {
          if (this.findCaseInsensitiveFromObj(table, 'Category') !== '' && this.findCaseInsensitiveFromObj(table, 'Category') !== null) {
            this.categoryService.searchByPath({keyword: this.findCaseInsensitiveFromObj(table, 'Category') + ' > '}).subscribe(
              data => {
                if (data.data.pathList.length > 0) {
                  mainCatHasSub = true;
                  this.categoryService.searchByPath({keyword: this.findCaseInsensitiveFromObj(table, 'Category') + ' > ' + this.findCaseInsensitiveFromObj(table, 'Sub Category') + ' > '}).subscribe(
                    subData => {
                      if (subData.data.pathList.length > 0) {
                        subCatHasSubs = true;
                      }
                    }
                  )
            ...
            if (data.data.pathList[0] === undefined || data.data.pathList[0] === null) {
              ...
              if (isSubSubCat) {
                ...
                if (lastIndex !== -1) {
                  ...
                  this.categoryService.searchByPath(payload2).subscribe(
                    subData => {
        ...
        // Check if seller income is 0
        // if (table.sellerIncome === 0) {
        //   marginRate.className = 'table-danger';
        //   sellerIncome.className = 'table-danger';
        //   verified = false;
        // }
        ...
        // //check brand
        // const brandValue = this.findCaseInsensitiveFromObj(table, 'Seller SKU');
        // if (brandValue !== undefined){
        //   if (brandValue === '' || brandValue === null){
        //     brand.className = 'table-danger';
        //     verified = false;
        //   }
        // } else {
        //   verified = false
