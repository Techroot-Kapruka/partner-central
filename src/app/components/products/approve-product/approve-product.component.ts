...
@Component({
  ...
  styleUrls: ['./approve-product.component.scss']
})
...
export class ApproveProductComponent implements OnInit {
  ...
  popupApproveImage(x: number) {
    switch (x) {
      case 1:
        this.imageUrl = this.imagePathURI + this.imageOne;
        this.modalRef = this.modalService.open(this.imagePopup, {centered: true});
        break;
      case 2:
        this.imageUrl = this.imagePathURI + this.imageOne2;
        this.modalRef = this.modalService.open(this.imagePopup, {centered: true});
        break;
      case 3:
        this.imageUrl = this.imagePathURI + this.imageOne3;
        this.modalRef = this.modalService.open(this.imagePopup, {centered: true});
        break;
      case 4:
        this.imageUrl = this.imagePathURI + this.imageOne4;
        this.modalRef = this.modalService.open(this.imagePopup, {centered: true});
        break;
      case 5:
        this.imageUrl = this.imagePathURI + this.imageOne5;
        this.modalRef = this.modalService.open(this.imagePopup, {centered: true});
        break;
  ...
  clearAllFeilds() {
    ...
    (document.getElementById('txt_condition') as HTMLInputElement).value = '';
...
// ===============================================================start edit methods====================================
  ...
  // Function to increase the displayed row count
  loadMore() {
  ...
  managetSelecedProductByEdit(data, proCodeImg) {
    ...
    if (data.data.product.has_group == 0) {
      ...
      for (let i = 0; i < data.data.product.productVariation.length; i++) {
        let variation = '';
        if (data.data.product.productVariation[i].variation_theme == 'color') {
          variation = getColorWord(data.data.product.productVariation[i].variation);
        } else {
          variation = data.data.product.productVariation[i].variation;
        }
        let payData = {
      ...
      for (let i = 0; i < data.data.product.productVariation.length; i++) {
        ...
        let variationTheame = '';
        let variation_theame_value = '';
        let variation_theame_value_concat_with_size = '';
        for (let j = 0; j < data.data.product.productVariation[i].variations.length; j++) {
          if (data.data.product.productVariation[i].variations[j].theame == 'Color') {
            variation = getColorWord(data.data.product.productVariation[i].variations[j].theame_value);
          } else {
            variation = data.data.product.productVariation[i].variations[j].theame_value;
          }
          variationTheame += variation + '-';

        }
        let payData = {
          id: data.data.product.productVariation[i].id,
          theme: variationTheame,
          value: variation,
    ...
    (document.getElementById('txt_condition') as HTMLInputElement).value = data.data.product.productOffer.condition;


    (document.getElementById('txt_description') as HTMLInputElement).innerHTML = data.data.product.productDescription.description;
  ...
  disableElement(data) {
    if (data.data.product.is_active == 1) {
      ...
      (document.getElementById('txt_condition') as HTMLInputElement).disabled = true;
      (document.getElementById('txt_amount') as HTMLInputElement).disabled = true;
      (document.getElementById('txt_price_rate') as HTMLInputElement).disabled = true;
